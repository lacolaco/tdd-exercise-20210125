...
describe('契約から売上（製品の価格）と収益認識を計算できる', () => {
    ...
    describe('収益認識は製品の種類に応じて計算される', () => {
        describe('ワードプロセッサは契約日に直ちに売上全額を収益認識する', () => {
            const products = [
                createProduct({ name: 'MS Word', price: 18800, category: 'ワードプロセッサ' }),
                createProduct({ name: '一太郎', price: 20000, category: 'ワードプロセッサ' })
            ];

            for (const product of products) {
                describe(`価格${product.price}円の製品「${product.name}」の収益認識を計算できる`, () => {
                    test('契約の収益認識は1回である', () => {
                        const contract = createContract({ product, signedDate: '2021-01-25' });
        ...
        describe('スプレッドシートは契約日に売上の 2/3、30 日後に 1/3 を収益認識する', () => {
            const products = [
                createProduct({ name: 'MS Excel', price: 27800, category: 'スプレッドシート' }),
                createProduct({ name: '三四郎', price: 5000, category: 'スプレッドシート' })
            ];
            ...
            for (const product of products) {
                describe(`価格${product.price}円の製品「${product.name}」の収益認識を計算できる`, () => {
                    ...
                    describe('収益認識の1つめは契約日に売上の 2/3を認識する', () => {
                        ...
                        test('認識金額は売上の2/3の端数を切り捨てた金額である', () => {
                            const contract = createContract({ product, signedDate: '2021-01-25' });
                            const revenueRecognition = contract.revenueRecognitions[0];
                            assert.equal(revenueRecognition.amount, Math.floor(contract.revenue * 2 / 3));
                        });
                    });
                    describe('収益認識の2つめは契約日の30日後に残りの売上を認識する', () => {
                        ...
                        test('認識金額は初回の認識額を引いた売上の残り1/3の金額である', () => {
                            const contract = createContract({ product, signedDate: '2021-01-25' });
                            const revenueRecognition = contract.revenueRecognitions[1];

                            assert.equal(revenueRecognition.amount, contract.revenue - Math.floor(contract.revenue * 2 / 3));
                        });
                    });
                    test('収益認識の総和は売上と完全一致する', () => {
                        const contract = createContract({ product, signedDate: '2021-01-25' });
        ...
        describe('データベースは契約日に売上の 1/3、60 日後に 1/3、120 日後に残りを収益認識する', () => {
            const products = [
                createProduct({ name: 'MS SQL Server', price: 919000, category: 'データベース' }),
                createProduct({ name: 'Oracle Database', price: 2100000, category: 'データベース' })
            ];
            ...
            for (const product of products) {
                describe(`価格${product.price}円の製品「${product.name}」の収益認識を計算できる`, () => {
                    ...
                    describe('収益認識の1つめは契約日に売上の 1/3を認識する', () => {
                        ...
                        test('認識金額は売上の1/3の端数を切り捨てた金額である', () => {
                            const contract = createContract({ product, signedDate: '2021-01-25' });
                            const revenueRecognition = contract.revenueRecognitions[0];
                            assert.equal(revenueRecognition.amount, Math.floor(contract.revenue * 1 / 3));
                        });
                    });
                    ...
                    describe('収益認識の3つめは契約日の120日後に残りの売上を認識する', () => {
                        ...
                        test('認識金額は売上の2/3を引いた残りの金額である', () => {
                            const contract = createContract({ product, signedDate: '2021-01-25' });
                            const revenueRecognition = contract.revenueRecognitions[2];
                            assert.equal(revenueRecognition.amount, contract.revenue - (Math.floor(contract.revenue * 1 / 3) + Math.floor(contract.revenue * 1 / 3)));
                        });
                    });
                    test('収益認識の総和は売上と完全一致する', () => {
                        const contract = createContract({ product, signedDate: '2021-01-25' });
